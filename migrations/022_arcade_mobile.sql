...
--              móvil de control de ventas de arcade
-- Fecha: 2026-02-08
-- =====================================================
...
CREATE TABLE IF NOT EXISTS arcade_daily_reports (
    ...
    -- Ventas por producto (JSON: [{product_id, product_name, quantity, price, total}])
    ...
    -- Gastos (JSON: [{description, amount}])
    expenses JSON,
...
-- 6. Agregar columna a la tabla de planes
-- Nota: Si la columna ya existe, este comando fallará pero se puede ignorar
ALTER TABLE plans 
ADD COLUMN mobile_module_enabled TINYINT DEFAULT 0 AFTER allow_modo_integration;

-- =====================================================
-- Datos de ejemplo para testing
-- =====================================================

-- Insertar configuración de ejemplo para tenant 1 (si existe)
INSERT INTO mobile_module_config (tenant_id, enabled, max_locations, max_employees_per_location)
SELECT 1, 1, 3, 2
FROM tenants
WHERE id = 1
LIMIT 1
...
-- Insertar local de ejemplo
INSERT INTO arcade_locations (tenant_id, location_name, address, active)
SELECT 1, 'Arcade Central', 'Av. Principal 123', 1
FROM tenants
WHERE id = 1
LIMIT 1;

-- Obtener ID del local recién creado
SET @location_id = LAST_INSERT_ID();

-- Insertar productos de ejemplo (fichas)
...
VALUES 
    (@location_id, 'empleado1', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'Juan Pérez', 20000.00, 1);

-- =====================================================
-- Verificación
-- =====================================================

SELECT 'Migración 022 completada exitosamente' AS status;
